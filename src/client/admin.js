...
export default () => {
  ...
  let compose = (...functr) => (inVal) => functr.reverse().reduce((acc, fn) => fn(acc), inVal)

  let colorLibGen = (data) => data.map((el) => Object.assign(el, { color: randomColor()} ))
  ...
  bars 
    ...
    .transition()
    .duration(900)
    .attr('width',  (el, i) => x(el.frequency))

  svg.selectAll('g') 
    .append('text')
    .attr('fill', 'white')
    .attr( 'y', (el, i) => dims.rectHeigth* (i + 1) - dims.barHeight/2 )
    .attr( 'x',  (el, i) => x(el.frequency) - 30 )
